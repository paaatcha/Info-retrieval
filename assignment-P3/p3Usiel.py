...
def setSingleFile (filesList):
    ...
    for fil in filesList:
        ...
        with io.open(fil,'r',encoding="utf-8") as f:
            text += unidecode(f.read())
            text += '\n</DOC>\n\n'                      
        
    with open('UsielCarneiroSingleFile.txt', 'w') as f:
        f.write(text)
...
def getIndexingZettair ():
    cmd = '/usr/local/zettair/bin/zet -i usielSingleFile.txt'
    os.system(cmd)    
